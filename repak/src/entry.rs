                ...
                match comp {
                    ...
                    Compression::Zstd => {
                            ...
                            decompressed.chunks_mut(chunk_size).zip(ranges)
                        {
                            ...
                            if framed.is_err() {
                                // Some licensee pak variants (e.g. Days Gone, unconfirmed)
                                // store raw/headerless Zstd blocks - no frame magic or
                                // descriptor, so the standard framed decoder rejects them
                                // with "Unknown frame descriptor". Each block's uncompressed
                                // size is already known from the pak index, so the raw block
                                // API is the correct fallback.
...
mod test {
    ...
    fn read_file_errors_instead_of_panicking_on_out_of_range_compression_slot() {
        let entry = super::Entry {
            ...
            timestamp: None,
            hash: Some(super::super::Hash([0; 20])),
            blocks: Some(vec![]),
            flags: 0,
        ...
        let ctx = super::EncryptionContext {
            key: &super::super::Key::None,
            variant: super::super::PakVariant::Standard,
            mount_point: "../../../",
            path: "test.uasset",
        };
        let mut out = vec![];
